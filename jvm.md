...
+ 链接
  ...
  + 解析：在类的常量池中寻找类、接口、字段和方法的符号引用，并把这些符号引用转换为直接引用
  
+ 初始化：为类的静态变量赋予正确的**初始值**

  <img src="C:\Users\14402\AppData\Roaming\Typora\typora-user-images\image-20200301161915846.png" alt="image-20200301161915846" style="zoom: 67%;" />

  + 初始化步骤

    + 如果该类还没有进行加载和链接，那就先进行加载和链接
    + 如果该类存在直接父类，且其父类还未初始化，则初始化其父类
    + 加入类中存在初始化语句，则依次执行这些初始化语句

  + 注意事项

    > 初始化一个类时要求其父类已被初始化，但是该规则不适用于接口
    >
    > + 初始化一个类，并不会先初始化该类实现的接口，**但是会加载，加载不到报错**
    > + 初始化一个接口，并不会初始化它的父接口，**但是会加载，加载不到报错**
    >
    > 因此一个接口并不会因为它的子接口或者实现类的初始化而初始化，只有当程序首次使用特定接口的静态变量（即运行时产生的值）时，才会导致该接口的初始化
    >
    > 调用一个接口的静态常量时并没有到初始化阶段，在编译期就把常量进行了替换，因此既不需要初始化父接口也不需要初始化本接口，访问父类的常量也是如此，也不需要加载。

> <font style="color:red">至此类的加载完成，这一动作的最终产品是位于内存中的Class对象，也是反射的入口</font>
>
> Class对象封装了类在方法区内的数据结构，并提供了访问这些数据的接口

+ 实例化
  + 为新的对象分配内存
  + 为实例变量赋予默认值
  + 为实例变量赋予初始值
  + java编译器为它编译的每一个类都至少生成一个**实例初始化方法**，在class文件中，这个**实例初始化方法**被称为`<init>`,针对源码中类的每一个构造方法都生成一个`<init>`方法
+ 卸载（可选）
...
+ 主动使用(七种)
  ...
  2. 访问某个类或接口静态变量，或者对类的静态变量赋值
     ...
     > + 如果该静态变量被定义为`static final`,那么在编译阶段，该常量会被存入到调用这个常量的方法所在的**类的常量池**中，调用类并没有直接引用这个定义这个常量的类，因此并不会触发定义常量的类的初始化，也不会加载该类
...
**类加载器并不需要某个类被首次主动使用时再加载该类，可以预先加载某个可能被使用的类，如果在预先加载的过程中遇到了.class文件缺失或存在错误，类加载器必须在首次主动使用该类时才报告错误（LinkageError），如果这个类一直没有被使用，那么类加载器就一直不报错**

### Java虚拟机自带的类加载器

+ 根类加载器（Bootstrap）

  > 没有父加载器，负责加载加载虚拟机核心类库，如`java.lang.*`等，`java.lang.Object`类就是由其加载。
  >
  > 它从系统属性<font style="color:red">`sun.boot.class.path`</font>指向的目录中加载类库，它依赖于底层操作系统，属于虚拟机筛选的一部分，没有继承<font style="color:red">`java.lang.ClassLoader`</font>类
...
从jdk1.2开始，类加载过程采用双亲委托机制，该机制能更好地保证Java平台的安全性。

在此机制中，除了根加载器外，其余的加载器都有一个父加载器。当Java程序请求`loader1`加载`Sample`类时，`loader1`首先委托自己的父加载器去加载`Sample`类，如果父加载器能完成加载，则由父加载器加载，否则由`loader1`完成加载
